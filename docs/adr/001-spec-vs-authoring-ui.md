...
* interpreters may define the state-relevant entities, attributes, predicates, and related concepts they support, typically with reference to one or more profiles
* authoring tools may load profiles and use them to infer, surface, and help maintain structural relationships that are not fully explicit in the raw text
...
* profiles should act as companion interpretation documents, not as a replacement for the raw specification
